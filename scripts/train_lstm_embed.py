# scripts/train_lstm_embed.py
# -- Walmart sales forecast: LSTM with Store/Dept embeddings + X standardization + log1p target --
# -- Train on seq_train.npz, validate on seq_valid.npz --
# -- Outputs:
# -- - output/lstm_embed_metrics.json
# -- - output/lstm_embed_pred_valid.csv
# -- - output/lstm_embed_preprocess_stats.json
...
class LSTMWithEmbeddings(nn.Module):
    def __init__(
        ...
        full_in = input_dim + store_emb_dim + dept_emb_dim
    ...
    def forward(self, x: torch.Tensor, store_id: torch.Tensor, dept_id: torch.Tensor) -> torch.Tensor:
        # -- x: (B, T, F)
        # -- store_id/dept_id: (B,)
        se = self.store_emb(store_id)  # (B, Es)
        de = self.dept_emb(dept_id)    # (B, Ed)

        B, T, _ = x.shape
        se_rep = se.unsqueeze(1).expand(B, T, se.shape[-1])
        de_rep = de.unsqueeze(1).expand(B, T, de.shape[-1])

        x2 = torch.cat([x, se_rep, de_rep], dim=-1)
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    with open(output_dir / "lstm_embed_metrics.json", "w", encoding="utf-8") as f:
    ...
    out_df.to_csv(output_dir / "lstm_embed_pred_valid.csv", index=False)
    ...
    with open(output_dir / "lstm_embed_preprocess_stats.json", "w", encoding="utf-8") as f:
        json.dump(stats, f, indent=2)

    print("[OK] LSTM(emb) training finished")
