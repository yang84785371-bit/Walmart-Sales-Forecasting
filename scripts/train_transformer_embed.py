# scripts/train_transformer_embed.py
# -- Walmart sales forecast: Transformer with Store/Dept embeddings + residual calibration --
# -- Design:
# -- - Transformer encodes temporal pattern from X (standardized)
# -- - Store/Dept embeddings provide global context and residual level calibration
# -- - log1p target, invert with expm1
# -- Outputs:
# -- - output/transformer_embed_metrics.json
# -- - output/transformer_embed_pred_valid.csv
# -- - output/transformer_embed_preprocess_stats.json
...
class TransformerEmbedResidual(nn.Module):
    def __init__(
        self,
        input_dim: int,
        max_len: int,
        ...
        # -- Map (store_emb, dept_emb) into d_model and add as global context to each token
        self.ctx = nn.Sequential(
            nn.Linear(store_emb_dim + dept_emb_dim, d_model),
            nn.Tanh(),
        )
        ...
        # -- Attention pooling instead of last token
        ...
        # -- Residual calibration from identity embeddings
    ...
    def forward(self, x: torch.Tensor, store_id: torch.Tensor, dept_id: torch.Tensor) -> torch.Tensor:
        ...
        w = self.pool_attn(h).squeeze(-1)  # (B, T)
        w = torch.softmax(w, dim=1).unsqueeze(-1)  # (B, T, 1)
        pooled = (h * w).sum(dim=1)  # (B, D)

        base = self.base_head(pooled).squeeze(-1)
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    # -- Smaller transformer tends to be more stable for short window=12
    parser.add_argument("--d_model", type=int, default=96)
    parser.add_argument("--nhead", type=int, default=4)
    parser.add_argument("--num_layers", type=int, default=2)
    parser.add_argument("--dim_ff", type=int, default=192)
    ...
    model = TransformerEmbedResidual(
    ...
    with open(output_dir / "transformer_embed_metrics.json", "w", encoding="utf-8") as f:
    ...
    out_df.to_csv(output_dir / "transformer_embed_pred_valid.csv", index=False)
    ...
    stats = {
        ...
        "dim_ff": args.dim_ff,
        "dropout": args.dropout,
        "arch": "transformer + ctx(add) + attention_pool + residual_calibration",
    }
    with open(output_dir / "transformer_embed_preprocess_stats.json", "w", encoding="utf-8") as f:
        json.dump(stats, f, indent=2)

    print("[OK] Transformer(emb_res) training finished")
